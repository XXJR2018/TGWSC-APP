...
@property (nonatomic, strong) CALayer *sliderMiddleLine;
...
- (void)initialiseControl {
    ...
    self.sliderMiddleLine = [CALayer layer];
    [self.layer addSublayer:self.sliderMiddleLine];
    
    //draw the minimum slider handle
    self.leftHandle = [CALayer layer];
    self.leftHandle.cornerRadius = 8.0f;
    self.leftHandle.borderWidth = 2;
    self.leftHandle.borderColor = [ResourceManager redColor1].CGColor;
    ...
    self.rightHandle.borderWidth = 2;
    self.rightHandle.borderColor = [ResourceManager redColor1].CGColor;
...
- (void)layoutSubviews {
    ...
    self.sliderLine.frame = CGRectMake(lineLeftSide.x, lineLeftSide.y, lineRightSide.x-lineLeftSide.x, 5.f);
//    self.sliderLine.backgroundColor = UIColorFromRGB(0xEFEFEF).CGColor;
    self.sliderLine.cornerRadius = 2.8f;
    
//    self.sliderMiddleLine.backgroundColor = [ResourceManager redColor1].CGColor;
    self.sliderMiddleLine.cornerRadius = 2.8f;
...
//    self.maxLabel.string = [formatter stringFromNumber:@(self.selectedMaximum)];
    self.minLabel.string = [NSString stringWithFormat:@"%@~%@",[formatter stringFromNumber:@(self.selectedMinimum)],[formatter stringFromNumber:@(self.selectedMaximum)]];
}
...
- (void)updateHandlePositions {
    ...
    if (self.disableRange) {
        leftHandleCenter.x = CGRectGetMinX(self.sliderLine.frame);
        
    }
    //中间已选区域颜色
    self.sliderMiddleLine.backgroundColor = [ResourceManager redColor1].CGColor;
    self.sliderMiddleLine.frame = CGRectMake(leftHandleCenter.x, leftHandleCenter.y - 2.0, rightHandleCenter.x-leftHandleCenter.x, 5.f);
...
#pragma mark - Animation 拖动时的缩放动画
...
-(void)setTintColor:(UIColor *)tintColor{
    ...
    self.leftHandle.backgroundColor = [UIColor whiteColor].CGColor;
    self.rightHandle.backgroundColor = [UIColor whiteColor].CGColor;
//    self.sliderMiddleLine.backgroundColor = color;
