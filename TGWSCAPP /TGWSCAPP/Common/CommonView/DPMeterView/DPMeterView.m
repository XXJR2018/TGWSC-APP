...
@interface DPMeterView ()<CAAnimationDelegate>
{
    int _starCount;             // 当前星星数量（点击评分时用到）
}
...
- (void)commonInit
{
    ...
    self.trackTintColor = UIColorFromRGB(0xC6C6C6);
    self.progressTintColor = UIColorFromRGB(0xFDB736);
...
-(void)tap:(UITapGestureRecognizer *)gesture{
    if(self.enable){
        ...
        float starWidth = self.frame.size.width/5.0;
        int count = (int)(point.x/starWidth)+1;
        
        if (count > _starCount) {
            [self add:(count-_starCount)/5.0];
        }else if (count == _starCount) {
        
        }else{
            [self minus:(_starCount - count)/5.0];
        }
