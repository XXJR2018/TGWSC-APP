...
-(void)drawRect:(CGRect)rect{
    ...
    NSLog(@"---- %@",[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"ZYQAssetPicker.Bundle/Images/AssetsPickerVideo@2x.png"]);
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (photosSelected && videoSelected)
        ...
        format = (indexPaths.count > 1) ? NSLocalizedString(@"已选择 %ld 张(最多%d张)", nil) : NSLocalizedString(@"已选择 %ld 张(最多%d张)", nil);
    
    else if (videoSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@"已选择 %ld 部视频", nil) : NSLocalizedString(@"已选择 %ld 部视频 ", nil);
    
    ZYQAssetPickerController *vc = (ZYQAssetPickerController *)self.navigationController;
    self.title = [NSString stringWithFormat:format, (long)indexPaths.count,vc.maximumNumberOfSelection];
}
...
{
    ZYQAssetPickerController *picker = (ZYQAssetPickerController *)self.navigationController;
    picker.navigationBar.barTintColor = [ResourceManager redColor2];
    UIColor *color = [UIColor whiteColor];
    picker.navigationBar.tintColor = color;
    picker.navigationBar.translucent = NO;
    picker.navigationBar.titleTextAttributes = [NSDictionary dictionaryWithObject:color forKey:UITextAttributeTextColor];
