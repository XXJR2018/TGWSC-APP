...
+ (UIImage *)imageNamedWithNoCache:(NSString *)name
{
    ...
    if (scale > 1.f)
    {
        ...
        if ((int)(scale*10)%10 != 0) {
            scaleStr = [NSString stringWithFormat:@"@%.1fx",scale];
        }else scaleStr = [NSString stringWithFormat:@"@%ldx",(long)scale];
        UIImage *image = [UIImage imageWithContentsOfFile:
                          [[NSBundle mainBundle]
                           pathForResource:DDG_str(@"%@%@", [name stringByDeletingPathExtension], scaleStr)
